...
async function loadGroupSettings(groupId) {
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            const defaults = {};
            await fs.writeFile(groupFilePath, JSON.stringify(defaults, null, 2), 'utf-8').catch(() => {});
            return defaults;
        }
...
async function createBotSocket(authDir) {
    try {
        ...
        NazunaSock.ev.on('connection.update', async (update) => {
            ...
            if (codeMode && !NazunaSock.authState.creds.registered && !pairingCodeRequested && connection === 'connecting') {
                pairingCodeRequested = true;
                setTimeout(async () => {
                    try {
                        let phoneNumber = (config.numerodono || '').replace(/\D/g, '');
                        if (!phoneNumber || !/^\d{10,15}$/.test(phoneNumber)) {
                            console.log('⚠️ Número inválido no config.json!');
                            return;
                        }
                        console.log(`☠️ Gerando código de pareamento para: +${phoneNumber}`);
                        const code = await NazunaSock.requestPairingCode(phoneNumber);
                        console.log(`\n☠️━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━☠️`);
                        console.log(`🔑 CÓDIGO DE PAREAMENTO: ${code}`);
                        console.log(`☠️━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━☠️`);
                        console.log('📲 Abra o WhatsApp > Aparelhos Conectados > Conectar > Digite o código acima.');
                    } catch (err) {
